...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'D_B', 'G_B',  'cycle_B','marker_reproduce_B','D_A1', 'G_A1', 'cycle_A1', 'D_B1', 'G_B1', 'cycle_B1']
        ...
        #SHUNXING
        self.visual_names = visual_names_A + visual_names_B + visual_names_A1 + visual_names_B1 # combine visualizations for A and B
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'G_A1', 'G_B1', 'D_A1', 'D_B1']
        else:  # during test time, only load Gs
        ...
        #        assert(opt.input_nc == opt.output_nc)
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters(), self.netG_A1.parameters(), self.netG_B1.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters(), self.netD_A1.parameters(), self.netD_B1.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_G(self):
        ...
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_marker_reproduce_B +self.loss_G_A1 +  self.loss_G_B1 + self.loss_cycle_A1 + self.loss_cycle_B1
